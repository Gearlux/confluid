...
| a **bare** activation of a dimension that carries a boolean block AND keyed blocks (`!scope:debug` beside `!scope:debug=verbose`) | the boolean block is selected; the keyed blocks stay off |
