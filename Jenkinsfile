                ...
                stage('Black') {
                    steps {
                        script {
                            ...
                            def targets = sh(script: "for d in confluid tests examples; do if [ -d \"\$d\" ] && find \"\$d\" -name '*.py' | grep -q .; then printf \"%s \" \"\$d\"; fi; done || true", returnStdout: true).trim()
                            if (targets) {
